...
public class SwerveDrive {
	...
	public SwerveDrive(
		...
		swerveDrivePoseEstimator = new SwerveDrivePoseEstimator(
				...
				new Pose2d(new Translation2d(0, 0),
						Rotation2d.fromDegrees(0))); // x,y,heading in radians; Vision measurement std dev, higher=less
														// weight
	...
	public void driveFieldOrientedandRobotOriented(ChassisSpeeds fieldOrientedVelocity,
		...
		ChassisSpeeds TotalVelocties = ChassisSpeeds
				.fromFieldRelativeSpeeds(fieldOrientedVelocity, getOdometryHeading())
	...
	public void drive(ChassisSpeeds velocity, boolean isOpenLoop, Translation2d centerOfRotationMeters) {
		...
		// Special handling for simulation to preserve translation speed when rotating
		if (SwerveDriveTelemetry.isSimulation) {
			// Check if we're both translating and rotating
			boolean isTranslating = Math.abs(velocity.vxMetersPerSecond) > 0.01
					|| Math.abs(velocity.vyMetersPerSecond) > 0.01;
			boolean isRotating = Math.abs(velocity.omegaRadiansPerSecond) > 0.01;
			...
			if (isTranslating && isRotating) {
				...
				// Custom desaturation that preserves translation speed better in simulation
				double maxDesiredSpeed = 0;
				for (SwerveModuleState moduleState : swerveModuleStates) {
					maxDesiredSpeed = Math.max(maxDesiredSpeed, Math.abs(moduleState.speedMetersPerSecond));
				}

				if (maxDesiredSpeed > maxSpeedMPS) {
					double scaleFactor = maxSpeedMPS / maxDesiredSpeed;
					for (int i = 0; i < swerveModuleStates.length; i++) {
						swerveModuleStates[i].speedMetersPerSecond *= scaleFactor;
					}
				}

				// Set the module states directly
				for (SwerveModule module : swerveModules) {
					module.setDesiredState(swerveModuleStates[module.moduleNumber], isOpenLoop, false);
				}
				return;
			}
		}
	 ...
	 * desaturateWheelSpeeds(SwerveModuleState[] moduleStates, ChassisSpeeds
	 * desiredChassisSpeed, double
	 * attainableMaxModuleSpeedMetersPerSecond, double
	 ...
	 * {@link SwerveDrive#setRawModuleStates(SwerveModuleState[], ChassisSpeeds, boolean)}
	 * function and
		...
		for (SwerveModule swerveModule : swerveModules) {
			...
			if (SwerveDriveTelemetry.verbosity.ordinal() >= TelemetryVerbosity.INFO.ordinal()) {
				...
				SwerveDriveTelemetry.desiredStates[(swerveModule.moduleNumber * 2)
						+ 1] = desiredState.speedMetersPerSecond;
			}
	...
	public void updateOdometry() {
		...
		try {
			...
			if (SwerveDriveTelemetry.verbosity.ordinal() >= TelemetryVerbosity.INFO.ordinal()) {
				...
				SwerveDriveTelemetry.measuredChassisSpeeds[2] = Math
						.toDegrees(measuredChassisSpeeds.omegaRadiansPerSecond);
			...
			for (SwerveModule module : swerveModules) {
				...
				if (SwerveDriveTelemetry.verbosity.ordinal() >= TelemetryVerbosity.INFO.ordinal()) {
					...
					SwerveDriveTelemetry.measuredStates[(module.moduleNumber * 2)
							+ 1] = moduleState.speedMetersPerSecond;
				}
			}
	 ...
	 *
	 * @param velocity                         The chassis speeds to set the robot
	 *                                         to achieve.
