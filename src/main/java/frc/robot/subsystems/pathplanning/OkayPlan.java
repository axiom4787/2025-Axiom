 ...
 * This implementation handles static obstacles from a navgrid file and accounts for robot dimensions.
 * Based on the paper: "OkayPlan: Obstacle Kinematics Augmented Dynamic real-time path Planning via particle swarm optimization"
...
public class OkayPlan implements Pathfinder {
        ...
        if (!navGridFile.exists()) {
            ...
            if (!navGridFile.exists()) {
                ...
                if (!navGridFile.exists()) {
                    ...
                    System.out.println("  - " + new File(Filesystem.getDeployDirectory(), "pathplanner/navgrid.json").getAbsolutePath());
                    System.out.println("  - " + new File("src/main/java/frc/robot/subsystems/pathplanning/navgrid.json").getAbsolutePath());
        ...
        try (BufferedReader br = new BufferedReader(new FileReader(navGridFile))) {
                        ...
                        if (polygon.containsKey("points")) {
                            ...
                            if (points != null && !points.isEmpty()) {
                                ...
                                if (polyPoints.size() >= 3) {
                                    ...
                                    int polygonId = polygon.containsKey("id") ? ((Number) polygon.get("id")).intValue() : -1;
                                    System.out.println("Added " + polygonType + " polygon with ID " + polygonId + " (" + polyPoints.size() + " points)");
                                }
                            }
                        }
                    }
            ...
            if (staticObstacles.size() > 10000) {
                ...
                for (GridPosition pos : staticObstacles) {
                    // Keep obstacles that are more critical (e.g., near edges, representing barriers)
    ...
    private Pair<List<GridPosition>, Boolean> planPathWithRetry(
        ...
        // Second attempt - try with relaxed obstacle checking (increase spacing between nodes)
     ...
     * This method combines A* for initial path finding with OkayPlan-inspired optimizations
    ...
    private Pair<List<GridPosition>, Boolean> planPath(
        ...
        for (int i = 0; i < waypointPath.size() - 1; i++) {
            ...
            Pair<List<GridPosition>, Boolean> segmentResult = 
                findPathSegmentAStar(segStart, segEnd, obstacles);
        ...
        // If we have enough points and we found a collision-free path, apply OkayPlan optimization
